...
def synthesize(node_contents):
    print(node_contents)
    out=momeutils.basic_task('Initial transcription: {}\n\n\n As an astute pitcher, make a more condensed and efficient version of the above, use the transcription language'.format(node_contents), model = 'g4o')
    print(out)
...
def bullets_from_synthesis(node_contents):
    print('='*10)
    print(node_contents)
    out = momeutils.basic_task('Target text: {}\n\n\n As an astute and efficient presenter, suggest between three and five concise bullet points that would support the presentation of the above, use the transcription language'.format(node_contents), model = 'g4o')
    print(out)
